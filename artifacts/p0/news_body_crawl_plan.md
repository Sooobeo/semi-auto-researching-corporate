...
P01의 뉴스 수집은 **기사 후보 URL 발견 → 원문 발행사 페이지 수집 → 본문 추출·검증**의 두 작업으로 나눈다. GDELT DOC와 NAVER 뉴스 검색 API는 후보를 찾는 데 쓴다. 두 API의 응답에 발행사 기사 전체 본문은 없으므로 `originallink`/발행사 URL을 따라가야 한다. 공시·IR 40건의 기간 커버리지와 뉴스의 사건 커버리지는 별도로 집계한다.
