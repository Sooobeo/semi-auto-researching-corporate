...
'''
    if (OUT / 'news_feasibility.md').is_file():
    ...
    if (OUT / 'news_body_manifest.csv').is_file():
        news_body = rows('news_body_manifest.csv')
        extracted = [row for row in news_body if row['location_status'] == 'source_xpath']
        blocks = sum(int(row['block_count']) for row in extracted)
        cells = sum(int(row['native_tables']) for row in extracted)
        review = sum(row['quality_status'] == 'needs_date_review' for row in news_body)
        report += (f'\n## 뉴스 본문 수집 시범\n\n'
                   f'공식 발행사 HTML {len(extracted)}건에서 원본 위치가 확인되는 본문 블록 '
                   f'{blocks}개를 추출했다. 표 {cells}개를 보존했고 날짜 검토 대상은 '
                   f'{review}건이다. 기사 후보 API와 발행사 본문 수집을 분리하는 방식은 '
                   f'[news_body_crawl_plan.md](news_body_crawl_plan.md), 개별 결과는 '
                   f'[news_body_manifest.csv](news_body_manifest.csv)에 기록했다.\n')
